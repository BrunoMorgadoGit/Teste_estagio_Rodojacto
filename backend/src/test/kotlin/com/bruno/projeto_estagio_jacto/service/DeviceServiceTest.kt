...
class DeviceServiceTest {
    ...
    @Test
    fun `operator should list only devices from its own organization`() {
        val service = DeviceService(deviceRepository, organizationService, authorizationService, DeviceMapper())
        val organization = Organization(id = 1L, corporateName = "Jacto A", registrationCode = "A-001")
        val devices = listOf(
            Device(id = 1L, model = "Device A", assetTag = "A-100", organization = organization),
        )

        given(authorizationService.isManager()).willReturn(false)
        given(authorizationService.getCurrentOrganizationId()).willReturn(1L)
        given(deviceRepository.findAllByOrganizationId(1L)).willReturn(devices)
    ...
    @Test
    fun `operator should not access device from another organization`() {
        val service = DeviceService(deviceRepository, organizationService, authorizationService, DeviceMapper())
        val organization = Organization(id = 2L, corporateName = "Jacto B", registrationCode = "B-001")
        val device = Device(id = 2L, model = "Device B", assetTag = "B-200", organization = organization)
